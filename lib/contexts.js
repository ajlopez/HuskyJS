...
function defineFunctionalFunctions(ctx) {
	ctx.set('.', function (xctx, x) {
		return function (yctx, y) {
			return function (zctx, z) {
                const xval = x.evaluate(xctx);
                const yval = y.evaluate(yctx);			
                const zval = z.evaluate(zctx);
